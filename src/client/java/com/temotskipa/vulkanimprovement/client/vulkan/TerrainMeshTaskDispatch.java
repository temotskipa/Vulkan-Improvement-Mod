...
record TerrainMeshTaskDispatch(int meshletOffset, int layerOrdinal, int requestedMeshlets, int taskCount,
    ...
    static TerrainMeshTaskDispatch fullLayer(int meshletOffset, int layerOrdinal, int meshletCount) {
        return new TerrainMeshTaskDispatch(meshletOffset, layerOrdinal, Math.max(meshletCount, 0),
                Math.clamp(meshletCount, 0, MAX_DIRECT_TASKS), 0L, 0L, 0L, 0L, 0, 0L, false,
                Source.DIRECT_LAYER, "");
    }
    ...
    static TerrainMeshTaskDispatch cpuVisibleList(DescriptorHeapTerrainResources.VisibleMeshletUpload upload, int layerOrdinal) {
        ...
        return new TerrainMeshTaskDispatch(upload.offset(), layerOrdinal, Math.max(upload.count(), 0),
                Math.clamp(upload.count(), 0, MAX_DIRECT_TASKS), upload.address(), 0L, 0L, 0L, 0, 0L, false,
                Source.CPU_VISIBLE_LIST, "");
    }
    ...
    static TerrainMeshTaskDispatch cpuWorkQueue(DescriptorHeapTerrainResources.TerrainWorkQueueUpload upload, int layerOrdinal, int requestedMeshlets) {
        ...
        return new TerrainMeshTaskDispatch(upload.offset(), layerOrdinal, requested, Math.min(upload.count(), MAX_DIRECT_TASKS),
                0L, upload.address(), 0L, 0L, 0, 0L, false, Source.CPU_WORK_QUEUE, "");
    }
    ...
    TerrainMeshTaskDispatch withIndirectCommand(DescriptorHeapTerrainResources.TerrainMeshTaskCommandUpload upload) {
        ...
        return new TerrainMeshTaskDispatch(this.meshletOffset, this.layerOrdinal, this.requestedMeshlets, this.taskCount,
                this.visibleMeshletListAddress, this.workQueueAddress, upload.vkBuffer(), upload.offsetBytes(),
                upload.strideBytes(), upload.address(), upload.gpuGenerated(), this.source, this.reason);
    }
    ...
    enum Source {
        ...
        DIRECT_LAYER("direct-layer", false, false),
        CPU_VISIBLE_LIST("cpu-visible-list", true, false),
        CPU_WORK_QUEUE("cpu-work-queue", false, true);
