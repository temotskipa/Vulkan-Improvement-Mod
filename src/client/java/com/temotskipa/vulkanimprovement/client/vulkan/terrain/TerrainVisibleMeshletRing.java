...
record TerrainVisibleMeshletRing(int capacity, int cursor) {
    ...
    record Allocation(int requestedRecords, int offset, int count, int capacity, int nextOffset, boolean wrapped,
                      boolean ready, boolean wrapBlocked, String reason) {
        Allocation {
            if (requestedRecords < 0) {
                requestedRecords = 0;
            }
