...
public final class ShaderBuildPipelineInvariantCheck {
    ...
    private static void checkReleaseBuildRequiresShaderValidator(String source) {
        ...
        require(source.contains("releaseBuild.get() || requireShaderValidator.get()"),
                "release builds must imply required glslangValidator checks");
    }
    ...
    private static void checkBothShaderTasksUseReleaseRequirement(String source) {
        ...
        require(compileTask.contains("inputs.property(\"releaseBuild\", releaseBuild)"),
                "SPIR-V generation task must track release-build mode as an input");
        require(compileTask.contains("requireTerrainShaderValidator()"),
                "SPIR-V generation task must fail without glslangValidator for release builds");
        require(checkTask.contains("inputs.property(\"releaseBuild\", releaseBuild)"),
                "terrain shader validation task must track release-build mode as an input");
        require(checkTask.contains("requireTerrainShaderValidator()"),
                "terrain shader validation task must fail without glslangValidator for release builds");
    }
